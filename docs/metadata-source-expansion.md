...
The first September 14 update incorrectly added `queue: max` and suppressed its validation error. The subsequent Joara catalog/resume workflows failed before starting any jobs. That setting has now been removed from every affected workflow. Workflow syntax and expression checks pass `actionlint` 1.7.12 without ignored diagnostics; a regression test permits only the supported concurrency keys. Workflow dispatch/publication remains untested during local validation. After pushing this correction, start a **new** workflow run; rerunning the original failed run uses its original commit.
