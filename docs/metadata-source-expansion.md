...
**Blank-title listings.** The blank-title works (for example `212133` and `1556465`) also have a blank `subject` in the public work-detail record, alongside an author and a completion flag, so no request can supply a title. Each omitted row now yields a minimal untitled record that the collector settles through the detail endpoint: a detail with a title becomes a normal record, and a blank detail title is recorded as an explicit, dated unavailability (like error 9001) rather than a retryable failure. Rows recorded as omitted by earlier scans are queued the same way at the start of a catalog or resume run, so they no longer wait for a fresh pass. Coverage counts a settled listing under `unavailable_rows` instead of `skipped_rows`, and settled listings no longer block `discovery_complete`; untitled records are never exported to the site. Explicit unavailability is re-checked after thirty days.
...
After discovery, per-category unique IDs observed in the current scan are compared with the category API totals. Mismatches remain explicit partial coverage; historical records from older scans cannot inflate the verification count. Saved records and translations remain additive. A complete sitemap scan is not reported as a complete catalog if that count check fails.

Live verification on September 16 enumerated 474,871 sitemap IDs and collected all 21,051 currently listed webnovels: Romance 1,682; Romance Fantasy 1,561; Fantasy 15,313; BL 2,495. Modern fantasy's 7,018 works were all present, including those missing from the overlapping sort-order approach. This added 9,315 records to the previous 11,736-record checkpoint. These counts describe that scan; each future scan verifies its own upstream totals.
