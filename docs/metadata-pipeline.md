...
Ridibooks (`ridi`) is the fourth metadata-v1 source and seventh platform. Its four webnovel genres, native weekly/monthly bestseller boards, workflows, translation, and artifact format use the common pipeline. Automated HTTP access currently returns 403 on the development host; live collection remains unverified. See [source scope and access status](metadata-source-expansion.md#ridibooks-integration-september-14). Joara latest lists now persist opaque continuation cursors; Resume migrates old numbered checkpoints while retaining records and translations.
