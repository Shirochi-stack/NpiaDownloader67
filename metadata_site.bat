...
if /i not "%METADATA_SOURCE%"=="naver" if /i not "%METADATA_SOURCE%"=="joara" if /i not "%METADATA_SOURCE%"=="munpia" goto :usage
...
:usage
echo Usage: metadata_site.bat naver^|joara^|munpia catalog^|rankings^|resume^|prepare^|translate^|merge^|build^|promote
