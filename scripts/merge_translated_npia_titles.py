...
Reads: docs/data/titles_untranslated.txt (with column 3 now filled in)
Updates: docs/data/titles_en.txt (preserves existing, adds new translations)
...
MAIN = os.path.join("docs", "data", "titles_en.txt")
PATCH = os.path.join("docs", "data", "titles_untranslated.txt")
...
for line in open(MAIN, "r", encoding="utf-8"):
    ...
    # Apply new translation if we don't already have one
