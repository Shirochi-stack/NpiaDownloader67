"""Build novelpia_top.json from novels.json for instant-load on the website.

Extracts the top weekly + monthly ranked Novelpia novels into a small standalone
JSON file with embedded translations and descriptions, enabling the website to
show results instantly while the full dataset loads in the background.
...
def main():
    ...
    output_path = os.path.join(base_dir, "novelpia_top.json")
        ...
        with open(trans_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(parts) >= 3 and parts[2].strip():
                    trans[parts[0]] = parts[2].strip()
        print(f"  {len(trans)} translations loaded")

    # Load descriptions (format: id|||description)
    descs = {}
